...
See [the manual QA checklist](docs/QA_CHECKLIST.md) for responsive, keyboard, and interaction checks. See [the demo script](docs/DEMO_SCRIPT.md) for a three-to-four-minute recording outline, and [the AWS deployment guide](docs/AWS_DEPLOYMENT.md) for the later cloud stage.
...
- The Gemini adapter has been tested with a real key and a valid structured response. Credentials remain local and are excluded from the source archive; other installations must provide their own key. Docker/cloud deployment and a screen-recorded demo are not claimed as completed by the local build.
...
| `APP_ORIGIN`       | `http://127.0.0.1:3000`; exact browser origin, no trailing slash |
| `PORT`             | Express port `4000`; web uses 3000                               |
